...
export class MonthlyAmmortizationReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.DigipayrollServiceService.GetEmployeeSalary().subscribe((data) => {
      ...
      this.uniquelist1 = [
        ...new Map(
          this.stafflist.map((item: { [x: string]: any }) => [item[key], item])
        ).values(),
      ];
    });
  }
  ...
  public getallemployee() {
    ...
    .subscribe({
      next: (data) => {
        debugger;
        this.stafflist1 = data.filter(
          (x) => x.loanType != null || x.loanType != 0
        );
        ...
        this.uniquelist2 = [
          ...new Map(
            this.stafflist1.map((item: { [x: string]: any }) => [
              item[key],
              item,
            ])
          ).values(),
  ...
  public FilterRoleType() {
    ...
    .subscribe({
      next: (data) => {
        ...
        this.uniquelist1 = [
          ...new Map(
            this.stafflist.map((item: { [x: string]: any }) => [
              item[key],
              item,
            ])
          ).values(),
        ...
        this.uniquelist1 = this.uniquelist1.filter(
          (x: { role: any }) => x.role == this.RoleType
        );
      },
      error: (err) => {
  ...
  public filterByDepartment() {
    ...
    .subscribe({
      next: (data) => {
        ...
        this.uniquelist1 = this.uniquelist1.filter(
          (x: { department_name: any }) => x.department_name == this.Department
        );
      },
      error: (err) => {
  ...
  public getemployee(id: any) {
    ...
    .subscribe({
      next: (data) => {
        debugger;
        this.stafflist1 = data.filter(
          (x) => x.staffID == id && x.loanType != null
        );
  ...
  selectALL(checked: boolean) {
    ...
    if (
      this.uniquelist1.every((val: { checked: boolean }) => val.checked == true)
    ) {
      this.uniquelist1.forEach((val: { checked: boolean }) => {
        val.checked = false;
      });
    } else {
      debugger;
          ...
          next: (data) => {
            ...
            this.results = this.stafflist13.map((val: { staffID: any }) => {
              return Object.assign(
                {},
                val,
                this.stafflist1.filter(
                  (v: { staffID: any }) => v.staffID == val.staffID
                )[0]
              );
            });
          },
    ...
    this.uniquelist2 = [
      ...new Map(
        this.results.map((item: { [x: string]: any }) => [item[key], item])
      ).values(),
    ];
